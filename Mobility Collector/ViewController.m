...
-(void) sendLoginURLRequest: (NSString*)username password:(NSString*)password{
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://pkks-dev.stis.ac.id/users/loginUser"]];
...
//                               NSLog(@"%@, %d", userId[@"id"], userId[@"id"]!=NULL);
                               
                               if (userId[@"id"]!=(id)[NSNull null]) {
                                   DatabaseHelper* dbHelper = [DatabaseHelper getInstance];
                                   [dbHelper setUserId:userId[@"id"] phoneModel:[self getPhoneModel] phoneOS: [self getSystemVersion]];
                                   
                                   // Preference
                                   NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
                                   [preferences setValue:username forKey:@"session_username"];
                                   [preferences setValue:password forKey:@"session_password"];
...
-(NSString*) getPhoneModel{
    ...
    if ([sDeviceModel isEqual:@"iPhone5,2"]) return @"iPhone5S";    //iPhone 5 (GSM+CDMA)
    if ([sDeviceModel isEqual:@"iPhone5,3"]) return @"iPhone5C";    //iPhone 5C (GSM)
    if ([sDeviceModel isEqual:@"iPhone5,4"]) return @"iPhone5C";    //iPhone 5C (Global)
    if ([sDeviceModel isEqual:@"iPhone6,1"]) return @"iPhone5S";    //iPhone 5S (GSM)
    if ([sDeviceModel isEqual:@"iPhone6,2"]) return @"iPhone5S";    //iPhone 5S (Global)
