        ...
        if ([filemgr fileExistsAtPath: databasePath ] == NO)
        {
            ...
            if (sqlite3_open(dbpath, &database) == SQLITE_OK)
            {
               // NSLog(@"Sucess open/create database");
                ...
                char *errMsg;
                const char *sql_stmt =
                "CREATE TABLE if not exists location_table (time_ bigint, userid integer, latitude double precision, longitude double precision, speed_ double precision, altitude double precision, bearing double precision, accuracy double precision, satellites_ integer, xMean double precision, yMean double precision, zMean double precision, totalMean double precision, xStdDev double precision, yStdDev double precision, zStdDev double precision, totalStdDev double precision, xMin double precision, yMin double precision, zMin double precision, totalMin double precision, xMax double precision, yMax double precision, zMax double precision, totalMax double precision, xNumberOfPeaks int, yNumberOfPeask int, zNumberOfPeaks int, totalNumberOfPeaks int, totalNumberOfSteps int, xIsMoving boolean, yIsMoving boolean, zIsMoving boolean, totalIsMoving boolean, size integer, upload boolean); CREATE TABLE if not exists admin_table (userid integer, phone_model text, phone_os text, isServiceOn boolean DEFAULT FALSE); Create table if not exists log_table(log_time text, log_message text)";
...
-(NSString *) getAllLocations{
    NSArray* dbLocs = [self executeJSONQuery:@"SELECT * FROM location_table"];
...
-(int)getLocationCount {
    const char *query_stmt = "Select count(*) from location_table";
    sqlite3_stmt*statement;
...
-(int)getToUploadLocationCount {
    const char *query_stmt = "Select count(*) from location_table WHERE upload=0";
    sqlite3_stmt*statement;
